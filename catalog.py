...
def likely_address(text: str) -> bool:
    ...
    markers = [
        ...
        "sector", "ঢাকা", "জেলা", "থানা", "রোড", "বাড়ি", "বাসা", "ফ্ল্যাট",
        "গ্রাম", "উপজেলা", "পোস্ট", "mirpur", "uttara", "dhaka", "cumilla",
        "chattogram", "sylhet", "khulna", "rajshahi", "barishal", "rangpur", "mymensingh"
    ]
    
    # Use regex word boundaries for English words to prevent "rd" from matching "order"
    for m in markers:
        # For Bengali words or long words, simple 'in' check is okay, but word boundary is safer for ascii
        if re.search(r'\b' + re.escape(m) + r'\b', t):
            return True
            
        # Fallback for Bengali words since \b might not work well with all unicode
        if any(char > '\u0980' for char in m) and m in t:
            return True
