...
async def process_event(event: Dict[str, Any]) -> None:
    ...
    if state["message_count"] == 1:
        ...
        if hook_products:
            ...
            hook_msg = (
                f"{prefix} আসসালামু আলাইকুম 🌸 আমাদের সবচেয়ে জনপ্রিয় প্রোডাক্ট দেখুন!"
            )
            await send_typing_indicator(psid)
            await send_facebook_text(psid, hook_msg)

            if product_name or price_text:
                caption = f"{product_name}\n{price_text}" if product_name and price_text else (product_name or price_text)
                try:
                    await send_facebook_text(psid, caption)
                except Exception as e:
                    logger.warning("[%s] Hook caption send failed: %s", psid, repr(e))
            ...
            follow_up = (
                f"{prefix} এটা নিতে চাইলে বলুন, অথবা অন্য কিছু খুঁজলে "
                "আপনার পছন্দের প্রোডাক্টের একটা স্ক্রিনশট পাঠান 🌸"
            )
            await send_facebook_text(psid, follow_up)

            # Save state and return — don't process the greeting through the full AI flow
