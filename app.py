...
""".strip()
    ...
    if order_complete(state) and not state.get("receipt_sent"):
        ...
        if state.get("cart"):
            price = sum(item.get("price_bdt", 0) * item.get("quantity", 1) for item in state["cart"])
            ...
            single_bodysuit_count = sum(
                ...
                if "bodysuit" in item.get("product_name", "").lower() and item.get("price_bdt", 0) < 600
            )
