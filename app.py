...
def apply_vision_to_state(state: Dict[str, Any], vision: Dict[str, Any]) -> bool:
    """
    Apply vision analysis to state. Returns True if a product match was found,
    False if no confident match (so the bot knows not to show a wrong product).
    """
    if not vision:
        return False
    ...
    if query_text and CATALOG:
        ...
        if candidates:
            ...
            # No confident match found — clear any previous match to avoid confusion
            logger.info("Vision search for '%s' found no confident match", query_text)
            return False
    return False
...
""".strip()
    ...
    if image_url:
        ...
        if image_bytes:
            ...
            vision_matched = apply_vision_to_state(state, vision)
            if not vision_matched and vision.get("product_query_text"):
                # Tell Gemini no match was found for what the customer sent
                vision["no_catalog_match"] = True

    # Track whether we have a confident product match from this message
    if not image_url:
        vision_matched = bool(state.get("matched_product_name"))
    # -------------------------------------------------------
    ...
    # -------------------------------------------------------
    if vision_matched and state.get("matched_image_url") and state.get("matched_product_name"):
        low = normalize_text(text)
        should_send_image = any(
            x in low for x in ["price", "dam", "দাম", "eta", "এটা", "same", "same color", "design"]
        )
        # Also send if customer sent an image and we found a confident match
        if image_url or should_send_image:
            try:
