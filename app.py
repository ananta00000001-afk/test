...
REPLY_SYSTEM_PROMPT = """
You are writing the FINAL customer-facing reply for a Bangla Messenger sales chat for a premium baby clothing store.

Rules:
- Return ONLY the final customer-facing message text.
- Do NOT return JSON, YAML, markdown, labels, bullets, placeholders, field names, or internal reasoning.
- Write ONLY in warm, natural conversational Bengali using Bangla script.
- Sound like a kind real Bangladeshi Messenger seller, not a robot.
- Keep replies short, but complete and clear. Use 1 or 2 short sentences.
...
- If the customer has corrected delivery zone or any detail, respect the corrected value.
...
- Correct wording examples: "বেবুর বয়সটা বলবেন?", "বেবুর বয়স মাসে কত?", "বেবুর বয়সটা জানালে আমি সাইজটা কনফার্ম করে দিচ্ছি 🌸"
- Wrong wording examples: "আপনি কোন বয়সের?", "আপনার বয়স কত?", "আপনি কত বছরের?"
- When the customer wants to order but size is still missing, ask about the baby's age in a warm and clear way.
- Do not make the reply so short that it becomes confusing.
...
def customer_wants_to_order(message_text: str, analysis: Dict[str, Any]) -> bool:
    txt = (message_text or "").lower()
    if analysis.get("user_intent") == "buy":
        return True
    keywords = [
        "order", "purchase", "confirm", "nite chai", "nite chaisi", "nibo", "kinbo",
        "order korte", "order korbo", "অর্ডার", "নিতে চাই", "নিতে চাচ্ছি", "কিনবো", "কিনতে", "পারচেস"
    ]
    return any(k in txt for k in keywords)


def build_missing_size_reply(state: Dict[str, Any]) -> str:
    style = state.get("addressing_style") or "আপু"
    if style == "ভাই":
        return "জ্বি ভাই, আমি অর্ডারটা এগিয়ে নিতে পারি 🌸 বেবুর বয়সটা বলবেন, আমি সঠিক সাইজটা কনফার্ম করে দিচ্ছি।"
    if style and style not in {"আপু", "ভাই"}:
        return f"জ্বি {style}, আমি অর্ডারটা এগিয়ে নিতে পারি 🌸 বেবুর বয়সটা বলবেন, আমি সঠিক সাইজটা কনফার্ম করে দিচ্ছি।"
    return "জ্বি আপু, আমি অর্ডারটা এগিয়ে নিতে পারি 🌸 বেবুর বয়সটা বলবেন, আমি সঠিক সাইজটা কনফার্ম করে দিচ্ছি।"
