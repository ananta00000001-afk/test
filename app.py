...
""".strip()
    ...
    if image_url:
        ...
        if image_bytes:
            ...
            try:
                ...
                logger.error("[%s] Visual match failed: %s", psid, repr(e))
                match_result = {}

            # Step 2: Secondary analysis (baby age/size hints)
            vision = analyze_image(image_bytes, mime, text)
            apply_vision_to_state(state, vision)

            # Step 3: FALLBACK — if visual match failed, try token-based search
    ...
    if state.get("address_pending") and not state.get("address") and not state.get("awaiting_confirmation"):
        ...
        prefix = addressing_prefix(state)
        await send_typing_indicator(psid)
        await send_facebook_text(psid, f"{prefix} আমি আপনার ঠিকানা হিসেবে নোট করেছি: {pending}।")
        return
    ...
    if state.get("phone_pending") and not state.get("phone") and not state.get("awaiting_confirmation"):
        ...
        prefix = addressing_prefix(state)
        await send_typing_indicator(psid)
        await send_facebook_text(psid, f"{prefix} আপনার কন্টাক্ট নাম্বার নোট করেছি: {pending}।")
        return
    ...
    if order_complete(state) and not state.get("receipt_sent"):
        ...
        phone = state.get("phone", "-")
        address = state.get("address", "-")
        price = state.get("matched_price_text", "-")
        
        receipt_text = f"অর্ডার সামারি:\nSize: {size}\nPhone: {phone}\nAddress: {address}\nPrice: {price}"
        await send_facebook_text(psid, receipt_text)
        
        if state.get("matched_image_url"):
            try:
                await send_facebook_image(psid, state.get("matched_image_url"))
            except Exception as e:
